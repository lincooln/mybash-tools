mybash-tools
Первый в мире проект, полностью написанный ИИ (Qwen3-Max) в соавторстве с человеком.
...
install-fonts.sh — умная установка шрифтов:
→ скачивает, очищает от мусора, устанавливает в систему, создаёт локальные архивы для offline-работы.
...
📚 Базы данных (data/)
help.txt — простой текстовый файл с подсказками. Формат:
тема команда # пояснение
Пример: vi :q! # выйти без сохранения
nerd-fonts.txt — база иконок Nerd Fonts. Формат:
U+XXXX;имя;тег1 тег2 ...
